...
const PostView: React.FC<FullPost> = ({ title, date, content, tags }) => {
    return (
        <div className={`${styles.post} mx-auto`}>
            ...
            <div className="my-12 mx-auto">
                ...
                <div className="pt-2 flex flex-col xs:flex-row xs:items-center text-gray-500 text-sm sm:text-base">
                    <div
                        className="hidden xs:block w-6 h-6 sm:w-12 sm:h-12 mr-1.5 sm:mr-3 rounded-full"
                        style={{
                ...
                {tags && (
                    <div className="flex space-x-2 sm:space-x-3 items-center mt-3 sm:mt-5 font-mono text-xs sm:text-sm">
...
const Tag = (tag: string) => {
    return (
        <Link href={`/blog/tags/${tag}`}>
            <a className="py-1 px-2 sm:py-1.5 border shadow-sm rounded">{`#${tag}`}</a>
        </Link>
    )
}
